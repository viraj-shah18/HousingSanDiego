...
function CollectionListComponent(props) {
  ...
  return (
    <Grid container spacing={2}>
      ...
      <Grid item xs={7} value={props.id} onClick={handleCollectionsClick} sx={clickable}>
      ...
      <Grid item xs={2}>
        <RemoveItem sx={clickable}>Remove</RemoveItem>
      </Grid>
    </Grid>
  )
};
...
export default function Collection () {
    ...
    function AddCollectionInput() {
      ...
      const [currMax, setMax] = useState(0);
      ...
      const sendTextInput = event => {
        ...
        setcollectionList([
          ...collectionsList,
          {id: currMax, name: textInput, num_places: 0, images: require("../home.png")}
        ]);
      }
    ...
    return (
      <Box sx={style}>
        ...
        <Box sx={{ flexGrow: 1, maxHeight: 300, overflow: 'auto' }}>
          {collectionsList.map(collection => (<CollectionListComponent id={collection.id} name={collection.name} num_places={collection.num_places} img={collection.images}/>))}
        </Box>
